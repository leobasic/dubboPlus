...
public class ServiceConfig<T> extends AbstractServiceConfig {
    ...
    private void doExportUrls() {
        ...
        for (ProtocolConfig protocolConfig : protocols) {
        	...
        	//thrift原生协议、dubbo thrift协议，不对非thrift生成的代码进行暴露，因为暴露了也没用
        	if (
        			(protocolConfig.getName().equals(ThriftNativeCodec.NAME) || protocolConfig.getName().equals(ThriftCodec.NAME)) 
            ...
            if (! Constants.SCOPE_LOCAL.toString().equalsIgnoreCase(scope) ){
                ...
                if (registryURLs != null && registryURLs.size() > 0
                    ...
                    for (URL registryURL : registryURLs) {
                        ...
                        //给注册中regitryUrl设置EXPORT_KEY值为前面构建的暴露服务url
                        //根据服务具体实现，实现接口以及regitryUrl从代理工厂ProxyFactory获取代理Invoker（继承于AbstractProxyInvoker）,它是对具体实现的一种代理
